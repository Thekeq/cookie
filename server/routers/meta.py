...
from server.settings import CHANNEL_USERNAME, bot_deep_link
...
async def auth(tg: dict = Depends(tg_user)):
    ...
    # Готовая реф-ссылка, а не имя бота: у startapp-ссылки две формы (см.
    # settings.bot_deep_link), и знать, какая из них настроена у бота, фронту
    # неоткуда — раньше он собирал URL сам и всегда собирал бесприставочную.
    # Собственный VITE_BOT_USERNAME не спасал и подавно: Vite зашивает env в
    # бандл НА СБОРКЕ и видит только префикс VITE_, так что BOT_USERNAME из
    # .env до ссылок не доходил вовсе.
    #
    # Именно в /auth, а не в full_state: тот возвращается из КАЖДОЙ изменяющей
    # ручки, и ссылка ездила бы в каждом ответе. Здесь она уезжает один раз за
    # сессию, ровно туда, где фронт её и запоминает.
    state["ref_link"] = bot_deep_link(f"ref_{tg['id']}")
    return state
