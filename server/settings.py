...
# Отдельная полка для метрик в Redis. Нужна ровно одному сценарию —
# канареечной выкладке: канарейка обязана делить с основными процессами
# лимитер и владение задачами (иначе лимит станет двойным, а задача пройдёт
# дважды), но НЕ метрики. Иначе её /metrics отдаст сумму по всему парку, где
# её собственные пятисотки размыты девяткой здоровых процессов, и вся затея
# «выкатили на 10% и посмотрели» превращается в «выкатили и не посмотрели».
METRICS_NAMESPACE = _s("METRICS_NAMESPACE")
